...
class Entity:
    ...
    def __init__(self, x: float, y: float, game_manager: GameManager, anim_sprite_path: str) -> None:
        ...
        self.anim_sprite_path = anim_sprite_path or "character/ow1.png"
        self.animation = Animation(
            f"{self.anim_sprite_path}", ["down", "left", "right", "up"], 4,
    ...
    def update(self, dt: float) -> None:
        ...
        if self.direction == Direction.DOWN:
            self.animation.cur_row = "down"
        if self.direction == Direction.LEFT:
            self.animation.cur_row = "left"
        if self.direction == Direction.RIGHT:
            self.animation.cur_row = "right"
        if self.direction == Direction.UP:
            self.animation.cur_row = "up"
    ...
    def camera(self) -> PositionCamera:
        ...
        '''
        camera_pos = Position(self.position.x,self.position.y)
        camera_pos.x -= GameSettings.SCREEN_WIDTH/2
        camera_pos.y -= GameSettings.SCREEN_HEIGHT/2
        return PositionCamera(int(camera_pos.x), int(camera_pos.y))
        
    def to_dict(self) -> dict[str, object]:
        return {
            "x": self.position.x / GameSettings.TILE_SIZE,
            "y": self.position.y / GameSettings.TILE_SIZE,
            "anim": self.anim_sprite_path
        }
    ...
    def from_dict(cls, data: dict[str, float | int], game_manager: GameManager) -> Entity:
        ...
        y = float(data["y"])
        anim = data["anim"]
        return cls(x * GameSettings.TILE_SIZE, y * GameSettings.TILE_SIZE, game_manager, anim)
