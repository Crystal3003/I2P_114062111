...
class ChatOverlay(UIComponent):
    ...
    def __init__(
        ...
        *,
        ...
        self._backspace_active = False
        self._backspace_timer = 0.0
        self._backspace_interval = 0.05
        self._backspace_delay = 0.5

        try:
            self._font_msg = pg.font.Font("assets/fonts/minecraft.ttf", 15)
            self._font_input = pg.font.Font("assets/fonts/minecraft.ttf", 20)
        except Exception:
    ...
    def _handle_typing(self, dt) -> None:
        # Turn keyboard keys into characters that appear inside the chat box.
        # Letters
        self._input_text = input_manager.text_buffer
        # Backspace
        if input_manager.key_pressed(pg.K_BACKSPACE):
            input_manager._delete_chr()
            self._backspace_timer = 0.0
            self._backspace_active = True
        if input_manager.key_released(pg.K_BACKSPACE):
            self._backspace_active = False
        if self._backspace_active and input_manager.key_down(pg.K_BACKSPACE):
            
            self._backspace_timer += dt
            if self._backspace_timer < self._backspace_delay:
                return
            while self._backspace_timer >= self._backspace_delay + self._backspace_interval:
                self._backspace_timer -= self._backspace_interval
                input_manager._delete_chr()
        # Enter to send
        if input_manager.key_pressed(pg.K_KP_ENTER) or input_manager.key_pressed(pg.K_RETURN):
    ...
    def draw(self, screen: pg.Surface) -> None:
        ...
        if self.is_open:
            msg_count = 12
            y = sh - 58 - 20 * msg_count
        else:
            msg_count = 5
            y = sh - 30 - 20 * msg_count
        msgs = self._get_messages(msg_count) if self._get_messages else [] 
        
        
        # Draw background for messages
        if msgs:
            container_w = max(100, int((sw - 20) * 0.6))
            bg = pg.Surface((container_w, 20 * msg_count), pg.SRCALPHA)
            bg.fill((0, 0, 0, 90 if self.is_open else 60))
            _ = screen.blit(bg, (x, y))
            # Render last messages
            lines = list(msgs)[-msg_count:]
            draw_y = y + 8
            for m in lines:
        ...
        text_surf = self._font_input.render(txt, 0, (255, 255, 255))
